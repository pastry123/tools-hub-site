...
import { Link, Unlink, Copy } from 'lucide-react';
...
export default function URLEncoder() {
  ...
  const handleEncode = async () => {
    ...
    try {
      const response = await fetch('/api/converter/url/encode', {
        method: 'POST',
        headers: {
  ...
  const handleDecode = async () => {
    ...
    try {
      const response = await fetch('/api/converter/url/decode', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
        body: JSON.stringify({ encoded: decodeInput })
      });
        ...
        <CardContent>
          <Tabs defaultValue="encode" className="w-full">
            ...
            <TabsContent value="decode" className="space-y-4 mt-6">
              <div>
                <Label htmlFor="decodeInput">URL to Decode</Label>
                <Textarea
                  id="decodeInput"
                  value={decodeInput}
